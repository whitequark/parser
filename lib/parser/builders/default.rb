module Parser
  ...
  class Builders::Default
    ...
    def parenthesize(begin_t, expr, end_t)
      if expr.nil? || synthesized_nil?(expr)
        # A nil expression, or a synthesized nil. Generally
        # both cases are generated by () or begin; end.
        n0(:nil,
          expr_map(loc(begin_t).join(loc(end_t))))
      else
        n(expr.type, expr.children,
          expr.src.with_begin_end(loc(begin_t), loc(end_t)))
      end
    end
